...
{
    private MfaChallenge? _pendingMfaChallenge;
    private string? _totpSessionInfo;
    private string? _totpEnrollmentId;
    ...
    private async Task SubmitMfaCodeAsync()
    {
        if (_pendingMfaChallenge is not { } challenge)
        {
            return;
        }
        ...
        try
        {
            var result = await authService.CompleteMfaSignInAsync(challenge, MfaCode.Trim());
            if (result.Succeeded)
            {
    ...
    private bool CanSubmit() => !IsBusy && Email.Trim().Length > 0 && Password.Length > 0;

    private bool CanSubmitMfaCode() => !IsBusy && MfaCode.Trim().Length > 0;
    ...
    [RelayCommand]
    private async Task RefreshSecurityStateAsync()
    {
        var state = await authService.RefreshSecurityStateAsync();
        if (state is null)
        {
            return;
        }
        ...
        _totpEnrollmentId = state.TotpEnrollmentId;
    }
    ...
    private async Task SendVerificationEmailAsync()
    {
        ...
        try
        {
            var result = await authService.SendEmailVerificationAsync();
    ...
    private async Task StartTotpEnrollmentAsync()
    {
        ...
        try
        {
            var start = await authService.StartTotpEnrollmentAsync();
            if (start is null)
            {
            ...
            _totpSessionInfo = start.SessionInfo;
    ...
    {
        if (_totpSessionInfo is not { } sessionInfo)
        {
            return;
        }
        ...
        try
        {
            var result = await authService.FinalizeTotpEnrollmentAsync(sessionInfo, TotpCode.Trim());
            if (result.Succeeded)
            {
                IsEnrollingTotp = false;
                TotpSecret = string.Empty;
                TotpCode = string.Empty;
                _totpSessionInfo = null;
    ...
    private bool CanConfirmTotpEnrollment() => !IsBusy && TotpCode.Trim().Length > 0;
    ...
    [RelayCommand]
    private async Task RemoveTotpEnrollmentAsync()
    {
        if (_totpEnrollmentId is not { } enrollmentId)
        {
            return;
        }
        ...
        try
        {
            var result = await authService.RemoveTotpEnrollmentAsync(enrollmentId);
            if (result.Succeeded)
            {
