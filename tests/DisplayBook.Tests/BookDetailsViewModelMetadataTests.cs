...
{
	static BookSummary Book(string title = "The Hobbit", string author = "J.R.R. Tolkien", string description = "Original description.") =>
		new(
			Id: "book-1",
			Title: title,
			Author: author,
	...
	static FetchedBookMetadata Fetched(string? title = "The Hobbit, or There and Back Again") =>
		new(
			Title: title,
	...
	static BookDetailsViewModel CreateViewModel(
		BookSummary book,
	...
	{
		FakeBookCatalogService catalog = new(Book());
		FakeBookMetadataService metadataService = new()
		{
			NextResult = new BookMetadataFetchResult(BookMetadataFetchStatus.Found, Fetched(), TimeSpan.FromMilliseconds(10), null),
		};
		BookDetailsViewModel viewModel = CreateViewModel(Book(), metadataService, catalog);
	...
	{
		BookSummary existingBook = Book() with { Isbn = "9780132350884" };
		FakeBookCatalogService catalog = new(existingBook);
		FakeBookMetadataService metadataService = new()
		{
			NextResult = new BookMetadataFetchResult(BookMetadataFetchStatus.Found, Fetched(), TimeSpan.FromMilliseconds(10), null),
		};
		BookDetailsViewModel viewModel = CreateViewModel(existingBook, metadataService, catalog);
		await viewModel.UpdateMetadataCommand.ExecuteAsync(null);
	...
	{
		FakeBookCatalogService catalog = new(Book(description: "Original description."));
		FakeBookMetadataService metadataService = new()
		{
			NextResult = new BookMetadataFetchResult(BookMetadataFetchStatus.Found, Fetched(), TimeSpan.FromMilliseconds(10), null),
		};
		BookDetailsViewModel viewModel = CreateViewModel(Book(description: "Original description."), metadataService, catalog);
		await viewModel.UpdateMetadataCommand.ExecuteAsync(null);

		MetadataFieldChange descriptionChange = Assert.Single(viewModel.ProposedChanges, c => c.Field == "description");
	...
	sealed class FakeBookMetadataService : IBookMetadataService
	{
	...
	sealed class FakeBookCatalogService(BookSummary book) : IBookCatalogService
	{
		BookSummary book = book;
		BookSummary? previous;

		public Task<IReadOnlyList<BookSummary>> GetBooksAsync(CancellationToken cancellationToken = default) =>
			Task.FromResult<IReadOnlyList<BookSummary>>([book]);

		public Task<BookSummary?> GetBookAsync(string bookId, CancellationToken cancellationToken = default) =>
			Task.FromResult(string.Equals(bookId, book.Id, StringComparison.Ordinal) ? book : null);
		...
		{
			previous = book;
			book = newCoverRelativePath is null ? updated : updated with { CoverPath = newCoverRelativePath };
