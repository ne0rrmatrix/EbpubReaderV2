...
public sealed class OpdsDownloadAndMembershipTests
{
    ...
    public void GetFileName_UsesReadableTitleUrlHashAndEpubExtension()
    {
        ...
        var fileName = DownloadFileNaming.GetFileName(link, "Elric: The Stealer of Souls");
    ...
    {
        var first = new DownloadLink
        {
            Url = "http://calibre.local:8012/get/epub/3819/Calibre_Library",
            Format = "application/epub+zip"
        };
        var second = new DownloadLink
        {
            Url = "http://calibre.local:8012/get/epub/3820/Calibre_Library",
            Format = "application/epub+zip"
        };

        var firstName = DownloadFileNaming.GetFileName(first, "First book");
        var secondName = DownloadFileNaming.GetFileName(second, "Second book");
